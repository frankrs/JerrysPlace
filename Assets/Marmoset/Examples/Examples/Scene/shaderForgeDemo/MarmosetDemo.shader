...
/////// Normals:
                float2 node_112 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_BumpTex,TRANSFORM_TEX(node_112.rg, _BumpTex))).rgb;
...
///////// Gloss:
                float4 node_42 = tex2D(_SpecularMap,TRANSFORM_TEX(node_112.rg, _SpecularMap));
...
////// Specular:
                ...
                finalColor += diffuseLight * (tex2D(_MainTex,TRANSFORM_TEX(node_112.rg, _MainTex)).rgb*_DiffuseColor.rgb);
                finalColor += specular;
/// Final Color:
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_BumpTex,TRANSFORM_TEX(node_113.rg, _BumpTex))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/////// Diffuse:
                ...
                float3 diffuse = max( 0.0, NdotL) * attenColor;
///////// Gloss:
                float4 node_42 = tex2D(_SpecularMap,TRANSFORM_TEX(node_113.rg, _SpecularMap));
                float node_56 = (node_42.a*_SpecularColor.a*_SpecularSharpness);
                float gloss = node_56;
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                ...
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * (tex2D(_MainTex,TRANSFORM_TEX(node_113.rg, _MainTex)).rgb*_DiffuseColor.rgb);
                finalColor += specular;
/// Final Color:
